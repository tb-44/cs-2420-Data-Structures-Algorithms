...
public class SavingsDetailsGui extends JFrame {

	private static final long serialVersionUID = 4365971203823305450L;
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					SavingsDetailsGui frame = new SavingsDetailsGui();
	...
	public SavingsDetailsGui() {
		...
		JLabel lblSavings = new JLabel(" Savings Details");
		...
		try {
			if (DB.hasTransaction(BankGui.gettextFieldUserName(), AccountTypes.SAVINGS)) {
				List<BankTransaction> txns = DB.readtxn(BankGui.gettextFieldUserName(), AccountTypes.SAVINGS);
